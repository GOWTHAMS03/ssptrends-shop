...
router.post('/add', async (req, res) => {
  try {
    const { name, business, phoneNumber, email, sizeName } = req.body;
...
router.put('/:id/active', auth, async (req, res) => {
  try {
    ...
    if (!update.isActive) {
      await deactivatesize(merchantId);
...
router.post('/signup/:token', async (req, res) => {
  try {
    ...
    await createMerchantsize(merchantDoc);
...
const deactivatesize = async merchantId => {
  const merchantDoc = await Merchant.findOne({ _id: merchantId }).populate(
    'size',
    '_id'
  );
  if (!merchantDoc || !merchantDoc.size) return;
  const sizeId = merchantDoc.size._id;
  const query = { _id: sizeId };
  const update = {
    isActive: false
  };
  return await size.findOneAndUpdate(query, update, {
    new: true
  });
};

const createMerchantsize = async ({ _id, sizeName, business }) => {
  ...
  const sizeDoc = await newsize.save();
